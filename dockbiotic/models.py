...
from dockbiotic.losses import AntibioticLoss, REDLoss


class AttentiveFP(nn.Module):
    """
    Class taken from DeepChem
    """
    ...
    def __init__(self,
        ...
        if mode not in ['regression', 'classification', 'antibiotic_classification', 'red']:
            raise ValueError("mode must be either 'regression', 'classification', 'antibiotic_classification' or 'red'")
...
class AttentiveFPModel(TorchModel):
    """
    Same model as in DeepChem, except for the addition of the mode 'antibiotic_classification'
    that uses the antibiotic loss.
    """
    ...
    def __init__(self,
        ...
        dropout: float
          Dropout probability. Default to 0.
        mode: str
          The model type, 'regression', 'classification' or 'antibiotic_classification'. Default to 'regression'.
        number_atom_features: int
          The length of the initial atom feature vectors. Default to 30.
        number_bond_features: int
          The length of the initial bond feature vectors. Default to 11.
        ...
        elif mode == 'antibiotic_classification':
            loss: Loss = AntibioticLoss(c_fp=c_fp, c_fn=c_fn)
            output_types = ['prediction']
        elif mode == 'classification':
            loss = SparseSoftmaxCrossEntropy()
            output_types = ['prediction', 'loss']
        elif mode == 'red':
            loss = REDLoss(r_slice=r_slice,e_slice=e_slice,d_slice=d_slice)
            output_types = ['prediction', 'loss']
